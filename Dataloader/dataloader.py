...
class FewshotDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for label_num in range(self.way_num):
            # trả về chỉ số (indices) của tất cả các phần tử trong tensor input có giá trị khác 0 (hoặc True nếu tensor Boolean)
            support_idxs = torch.nonzero(
                train_label_gpu[label_indices] == label_num, as_tuple=False
            ).flatten()
            support_idxs = support_idxs[: self.shot_num]
            support_data = train_data_gpu[label_indices][support_idxs]

            query_idxs = torch.nonzero(
                train_label_gpu[label_indices] == label_num, as_tuple=False
            ).flatten()
            query_idxs = query_idxs[~torch.isin(query_idxs, support_idxs)][
                : self.query_num
            ]
